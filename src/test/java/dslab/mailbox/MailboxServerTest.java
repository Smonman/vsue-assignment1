...
public class MailboxServerTest extends TestBase {
    ...
    private static final Log LOG =
        LogFactory.getLog(MonitoringServerTest.class);

    @Test
    public void runAndShutdownTransferServer_createsAndStopsTcpSocketCorrectly()
        throws Exception {
        String componentId = "mailbox-earth-planet";
        Config config = new Config(componentId);

        IMailboxServer component =
            ComponentFactory.createMailboxServer(componentId, in, out);
        ...
        try {
            ...
            Sockets.waitForSocket("localhost", dmtpPort,
                Constants.COMPONENT_STARTUP_WAIT);
        } catch (SocketTimeoutException e) {
            err.addError(new AssertionError(
                "Expected a TCP server socket on port " + dmtpPort, e));
        }

        try {
            LOG.info("Waiting for DMAP socket to open on port " + dmapPort);
            Sockets.waitForSocket("localhost", dmapPort,
                Constants.COMPONENT_STARTUP_WAIT);
        } catch (SocketTimeoutException e) {
            err.addError(new AssertionError(
                "Expected a TCP server socket on port " + dmapPort, e));
        }
        ...
        try {
            ...
            err.addError(new AssertionError(
                "Monitoring server was not terminated correctly"));
        }


        err.checkThat("Expected tcp socket on port " + dmtpPort +
                " to be closed after shutdown",
            Sockets.isServerSocketOpen(dmtpPort), is(false));

        err.checkThat("Expected tcp socket on port " + dmapPort +
                " to be closed after shutdown",
