...
public class MonitoringServerProtocolTest extends TestBase {
    ...
    private static final Log LOG =
        LogFactory.getLog(MonitoringServerProtocolTest.class);
    ...
    public void setUp() throws Exception {
        component =
            ComponentFactory.createMonitoringServer(componentId, in, out);
        addr = new InetSocketAddress("127.0.0.1",
            new Config(componentId).getInt("udp.port"));
    ...
    public void addresses_returnsCorrectStatistics() throws Exception {
        ...
        try (DatagramSocket socket = new DatagramSocket()) {
            ...
            socket.send(
                new DatagramPacket(str1.getBytes(), str1.length(), addr));
            socket.send(
                new DatagramPacket(str2.getBytes(), str2.length(), addr));
            socket.send(
                new DatagramPacket(str3.getBytes(), str3.length(), addr));
        }
