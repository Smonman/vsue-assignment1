...
public class MonitoringServerTest extends TestBase {
    ...
    @Test
    public void runAndShutdownMonitoringServer_createsAndStopsUdpSocketCorrectly()
        throws Exception {
        IMonitoringServer component =
            ComponentFactory.createMonitoringServer("monitoring", in, out);
        ...
        Thread.sleep(
            Constants.COMPONENT_STARTUP_WAIT); // wait a bit for resources to be initialized

        try {
            LOG.info("Trying to create socket on port " + port);
            err.checkThat("Expected an open UDP socket on port " + port,
                Sockets.isDatagramSocketOpen(port), is(true));
        } catch (Exception e) {
        ...
        err.checkThat("Expected datagram socket on port " + port +
